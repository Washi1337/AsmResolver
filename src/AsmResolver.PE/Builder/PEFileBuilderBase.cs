...
namespace AsmResolver.PE.Builder
{
    ...
    /// </summary>
    public abstract class PEFileBuilderBase : IPEFileBuilder
    {
        /// <inheritdoc />
        public virtual PEFile ConstructPEFile(IPEImage image)
        {
            ...
            foreach (var section in CreateSections(image))
                peFile.Sections.Add(section);
            
            ComputeHeaderFields(peFile, image);
        ...
        protected abstract IEnumerable<PESection> CreateSections(IPEImage image);
        ...
        protected abstract IEnumerable<DataDirectory> CreateDataDirectories(PEFile peFile, IPEImage image);
        ...
        protected abstract uint GetEntrypointAddress(PEFile peFile, IPEImage image);
        ...
        protected abstract uint GetFileAlignment(PEFile peFile, IPEImage image);
        ...
        /// the architecture.
        /// </returns>
        protected abstract uint GetSectionAlignment(PEFile peFile, IPEImage image);
        ...
        protected abstract uint GetImageBase(PEFile peFile, IPEImage image);
        ...
        /// <param name="image">The image that the PE file was based on.</param>
        protected virtual void ComputeHeaderFields(PEFile peFile, IPEImage image)
        {
            ComputeOptionalHeaderFields(peFile, image);
            ComputeFileHeaderFields(peFile, image);
        ...
        /// <param name="peFile">The (incomplete) PE file to update.</param>
        /// <param name="image">The image that the PE file was based on.</param>
        protected virtual void ComputeOptionalHeaderFields(PEFile peFile, IPEImage image)
        {
            var header = peFile.OptionalHeader;

            header.ImageBase = GetImageBase(peFile, image);
            header.SectionAlignment = GetSectionAlignment(peFile, image);
            header.FileAlignment = GetFileAlignment(peFile, image);
            ...
            header.AddressOfEntrypoint = GetEntrypointAddress(peFile, image);
            ...
            var dataDirectories = CreateDataDirectories(peFile, image).ToList();
        ...
        /// <param name="peFile">The (incomplete) PE file to update.</param>
        /// <param name="image">The image that the PE file was based on.</param>
        protected virtual void ComputeFileHeaderFields(PEFile peFile, IPEImage image)
        {
