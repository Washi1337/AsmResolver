...
namespace AsmResolver.DotNet.Signatures
{
    internal static class TypeNameParser
    {
        public static TypeSignature ParseType(ModuleDefinition parentModule, string name)
        {
            ...
            var elementType = (TypeReference) type.GetUnderlyingTypeDefOrRef();
            ...
            elementType.Scope = ReadAssemblyReference(name, ref position);
        ...
        private static AssemblyReference ReadAssemblyReference(string name, ref int position)
        {
            ...
            return new AssemblyReference(assemblyName, version, false, token)
            {
                Culture = culture
            };
        }
