...
/// <summary>
/// Provides a lazily initialized implementation of <see cref="InstanceDataMember"/> that is read from a PDB image.
/// </summary>
public class SerializedInstanceDataMember : InstanceDataMember
{
    ...
    public SerializedInstanceDataMember(PdbReaderContext context, uint typeIndex, ref BinaryStreamReader reader)
        : base(typeIndex)
    {
    ...
    /// <inheritdoc />
    protected override CodeViewType? GetDataType()
    {
        if (_dataTypeIndex == 0)
            return null;

        return _context.ParentImage.TryGetLeafRecord(_dataTypeIndex, out var leaf) && leaf is CodeViewType type
            ? type
